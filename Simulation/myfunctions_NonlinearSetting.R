run.one <- function(seed, n.units, t, t0, theta, true.beta,
                    addcov = F,
                    dist.epsilon){
  set.seed(seed)
  n.ctrl <- n.units - 1
...
generate.U <- function(n.units, t){
  ...
  lambda.t <- cbind(replicate(n = n.W, gen.lambda(t)))
...
NC.U0 <- function(para, data){
  ...
  bridge <- exp(
    X * beta + W %*% omega.u + Comega * theta.u    ## average rate ratio
  )
  NC.obj <- c(Y - bridge) * (instrument)
  return(NC.obj)
}
